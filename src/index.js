...
class NTLMAuthProvider {
  ...
  constructor(connection: any, options: Options) {
    this.connection = connection;
    this.options = options;
  }

  handshake(data: ?Buffer, callback: (error: ?Error, value: ?Buffer) => void) {
    if (!data) {
  ...
  parseChallengeMessage(buffer: Buffer) {
    const challenge = {};
  ...
  buildAuthenticateMessage(challenge: { target: Buffer, nonce: Buffer, flags: number }) {
  ...
  createTimestamp(time: number) {
    const tenthsOfAMicrosecond = new BigNumber(time).plus(11644473600).multiply(10000000);
    const hexArray = [];
  ...
  buildLMv2Response(responseKey: Buffer, serverNonceBuffer: Buffer, clientNonceBuffer: Buffer) {
  ...
  buildNTLMv2Response(responseKey: Buffer, targetInfo: Buffer, timestampBuffer: Buffer, serverNonceBuffer: Buffer, clientNonceBuffer: Buffer) {
  ...
  generateResponseKey(domain: ?string, username: string, password: string) {
